...
def install_deps_debian(*, assume_yes: bool = True) -> None:
    if not host_is_debian_like():
        raise RuntimeError("Host is not detected as Debian/Ubuntu; install deps manually.")
    pkgs = [
        "qemu-kvm","libvirt-daemon-system","libvirt-clients","virtinst",
        "cloud-image-utils","qemu-utils","curl","openssh-client","iproute2","nftables"
    ]
    ...
    run_cmd(["systemctl","enable","--now","libvirtd"], sudo=True, check=False, capture=True)
