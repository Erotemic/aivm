...
def test_dryrun_commands_with_yes(tmp_path: Path) -> None:
    ...
    commands = [
        ...
        ["host", "net", "create", "--yes", "--dry_run", "--config", str(cfg_path)],
        ["host", "net", "destroy", "--yes", "--dry_run", "--config", str(cfg_path)],
        ["host", "fw", "apply", "--yes", "--dry_run", "--config", str(cfg_path)],
        ["host", "fw", "remove", "--yes", "--dry_run", "--config", str(cfg_path)],
        ...
        ["vm", "sync_settings", "--yes", "--dry_run", "--config", str(cfg_path)],
    ]
    for argv in commands:
        assert _run(argv) == 0


def test_help_tree_includes_one_line_descriptions(tmp_path: Path, capsys) -> None:
    cfg_path = _write_cfg(tmp_path)
