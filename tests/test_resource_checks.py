...
from aivm.resource_checks import vm_resource_impossible_lines, vm_resource_warning_lines
...
def test_impossible_uses_memtotal_not_memavailable(monkeypatch) -> None:
    ...
    monkeypatch.setattr('aivm.resource_checks.host_mem_available_mb', lambda: 570)
...
def test_warning_uses_memtotal_and_ignores_low_memavailable(
    ...
    monkeypatch.setattr('aivm.resource_checks.host_mem_total_mb', lambda: 8192)
    monkeypatch.setattr('aivm.resource_checks.host_mem_available_mb', lambda: 488)
    monkeypatch.setattr('aivm.resource_checks.host_cpu_count', lambda: 8)
    monkeypatch.setattr('aivm.resource_checks.host_free_disk_gb', lambda p: 200.0)
