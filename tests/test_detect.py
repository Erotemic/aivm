...
def test_auto_defaults_sets_network_and_identity(monkeypatch, tmp_path: Path) -> None:
    cfg = AgentVMConfig()
    ...
    monkeypatch.setattr(
        "aivm.detect.detect_ssh_identity", lambda: ("/tmp/id_a", "/tmp/id_a.pub")
    )
    monkeypatch.setattr("aivm.detect.pick_free_subnet", lambda preferred: "10.88.0.0/24")
