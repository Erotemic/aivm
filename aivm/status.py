...
def probe_vm_state(cfg: AgentVMConfig, *, use_sudo: bool) -> tuple[ProbeOutcome, bool]:
    dom = run_cmd(
    ...
    return ProbeOutcome("running" in state.lower(), f"{cfg.vm.name} state={state}"), True
...
def render_status(
    ...
    *,
    ...
    img_ok = (
        run_cmd(["test", "-f", str(base_img)], sudo=use_sudo, check=False, capture=True).code
        == 0
    )
    if use_sudo:
        total += 1
        ...
        lines.append(
            status_line(None, "Base image cache", f"skipped without --sudo ({base_img})")
        )
    ...
    if ip and not vm_defined:
        lines.append(status_line(False, "Cached VM IP", f"{ip} (stale: VM not defined)"))
        ip = None
    else:
        lines.append(status_line(bool(ip), "Cached VM IP", ip or "no cached IP yet"))
    ...
    if vm_out.ok is True and ip and ssh.ok:
        ...
        prov = ProbeOutcome(
            None,
            "waiting for SSH" if cfg.provision.enabled else "disabled in config",
            "",
        )
...
def render_global_status() -> str:
    ...
    lines.append("Use `aivm config init --config .aivm.toml` or `aivm status --vm <name>`.")
    return "\n".join(lines)
