"""Pure schema helpers for ssh-agent repository credentials."""
...
def agent_credential_id(
    ...
    """Return the disjoint id used by ssh-agent repository grants."""
    payload = (
...
def validate_agent_credential_id_format(value: str) -> str:
    ...
    if not _AGENT_CREDENTIAL_ID_RE.fullmatch(text):
        raise CredentialValidationError(
            f'Invalid ssh-agent credential id {text!r}; expected '
            'agent-git-[0-9a-f]{12}.'
        )
    return text
...
def validate_agent_credential_identity(
    *,
    ...
    """Validate one ssh-agent credential's stable identity."""
    ...
    if validated_id != expected:
        raise CredentialValidationError(
            f'SSH-agent credential id {validated_id!r} does not match VM '
