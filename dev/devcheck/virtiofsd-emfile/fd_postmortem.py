...
def select_hot_pid(
    procs: list[VirtiofsdProc], vm: str | None, pid: int | None
) -> int:
    if pid is not None:
        return pid
    candidates = procs
    ...
    if not candidates:
        candidates = [
            p for p in procs if p.source and 'persistent-root' in p.source
        ]
    if not candidates:
        candidates = procs
    if not candidates:
...
def aggregate_targets(
    ...
    for target in targets:
        ...
        if target.startswith('/hostcode-'):
            ...
            if len(parts) >= 4:
                counters['by_token_topdir_seconddir'][
                    '/' + '/'.join(parts[1:4])
                ] += 1
            else:
            ...
            counters['other_special_kinds'][
                'absolute-non-hostcode:' + root
            ] += 1
        else:
...
def write_counter(
    path: Path, counter: collections.Counter[str], limit: int = 80
) -> None:
...
def render_aggregate_report(
    ...
    *,
    ...
    for name in [
        ...
        if name in counters:
            lines.append(
                render_counter_section(name, counters[name], limit=report_limit)
            )
    lines.append('')
    return '\n'.join(lines)
...
def write_aggregate_bundle(
    ...
    *,
    ...
    for name, counter in counters.items():
        write_counter(
            out / f'aggregate-{name}.tsv', counter, limit=aggregate_limit
        )
...
def render_inventory(procs: list[VirtiofsdProc]) -> str:
    lines = []
    lines.append(
        'PID\tPPID\tFDS\tNOFILE_SOFT\tNOFILE_HARD\tCAP_EFF\tCAP_BND\tSOURCE\tCMDLINE'
    )
    for p in procs:
        lines.append(
            '\t'.join(
...
def command_collect(args: argparse.Namespace) -> int:
    ...
    summary_lines.append(
        f'date: {_datetime.datetime.now().astimezone().isoformat()}'
    )
...
def command_watch(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        fds = count_fds(pid)
        print(
            f'{_datetime.datetime.now().astimezone().isoformat()}\t{pid}\t{fds}\t{soft}\t{hard}',
            flush=True,
        )
...
def command_decode_caps(args: argparse.Namespace) -> int:
    ...
    for key in ['CapInh', 'CapPrm', 'CapEff', 'CapBnd', 'CapAmb']:
        ...
        if value:
            ...
            if key == 'CapEff':
                print(
                    f'  has CAP_DAC_READ_SEARCH: {"CAP_DAC_READ_SEARCH" in names}'
                )
    return 0
...
def command_guest_drop_caches(args: argparse.Namespace) -> int:
    if os.geteuid() != 0:
        raise SystemExit(
            'guest-drop-caches must run as root inside the guest, e.g. sudo python3 ...'
        )
    print('sync()')
    os.sync()
    for value in args.values:
        ...
        try:
            ...
            if ex.errno == errno.EROFS:
                raise SystemExit(
                    '/proc/sys/vm/drop_caches is read-only in this environment'
                ) from ex
            raise
        if args.sleep:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        'collect', help='Collect a host-side incident evidence bundle'
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help='virtiofsd PID; default is busiest persistent-root worker',
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help='limit FD target audit for quick samples; 0 means all',
    )
    p.add_argument(
        '--aggregate-limit',
        type=int,
        default=80,
        help='rows to write in each aggregate TSV',
    )
    p.add_argument(
        '--report-limit',
        type=int,
        default=40,
        help='rows to show in the shareable text report',
    )
    p.add_argument(
        '--no-print-report',
        dest='print_report',
        action='store_false',
        help='do not print the shareable aggregate report to stdout',
    )
    p.set_defaults(func=command_collect, print_report=True)

    p = sub.add_parser(
        'aggregate',
        help="Aggregate an existing fd-targets.tsv or 'fd -> target' file",
    )
    p.add_argument('fd_targets')
    p.add_argument(
        '--out',
        default=None,
        help='output directory; default is a timestamped subfolder next to fd_targets',
    )
    p.add_argument(
        '--aggregate-limit',
        type=int,
    ...
    p.add_argument(
        ...
        dest='print_report',
        action='store_false',
        help='write report files but do not print the report to stdout',
    )
    ...
    p = sub.add_parser(
        'token-count', help='Count FD targets under one /hostcode-* token'
    )
    p.add_argument(
        'token', help='token name, e.g. hostcode-crfm-helm-audit-st-8bd60e77'
    )
    p.add_argument('--vm', default=os.environ.get('AIVM_VM', 'aivm-2404'))
    p.add_argument('--pid', type=int, default=None)
    ...
    p = sub.add_parser(
        'decode-caps', help='Decode Linux capability hex fields for a process'
    )
    ...
    p = sub.add_parser(
        'guest-drop-caches',
        help='Guest-side incident experiment: sync and drop caches',
    )
    p.add_argument(
        '--values',
        type=int,
        nargs='+',
        default=[2, 3],
        help='drop_caches values to write',
    )
    p.add_argument(
        '--sleep', type=float, default=30.0, help='seconds between writes'
    )
