...
earlier [virtiofsd_emfile_findings.md](virtiofsd_emfile_findings.md) notes.
...
This overlay adds [virtiofsd_fd_postmortem.py](virtiofsd_fd_postmortem.py), a
...
```bash
sudo python3 dev/devcheck/virtiofsd_fd_postmortem.py collect \
    --vm aivm-2404 \
...
```bash
python3 dev/devcheck/virtiofsd_fd_postmortem.py aggregate \
...
```bash
sudo python3 dev/devcheck/virtiofsd_fd_postmortem.py watch \
    --vm aivm-2404 \
    --interval 5 \
    | tee fd-watch.txt
```
...
```bash
sudo python3 dev/devcheck/virtiofsd_fd_postmortem.py token-count \
...
```bash
sudo python3 dev/devcheck/virtiofsd_fd_postmortem.py guest-drop-caches
```
...
```bash
sudo python3 dev/devcheck/virtiofsd_fd_postmortem.py decode-caps 350482
```
...
1. Convert `virtiofsd_fd_postmortem.py collect` output into an `aivm doctor` or
