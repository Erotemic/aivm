...
def _read_proc_limits(pid):
    ...
    try:
        ...
        return {
            '_error': f'PermissionError reading /proc/{pid}/limits (try sudo)'
        }
    out = {}
...
def _get_argv(pid):
    try:
        ...
        return []
    return [
        p.decode('utf-8', errors='replace') for p in raw.split(b'\x00') if p
    ]
    ...
    try:
        for i, fpath in enumerate(files_iter):
            ...
            try:
                ...
                print(
                    f'[GUEST]    errno:          {errno.errorcode.get(ex.errno, ex.errno)}'
                )
            ...
            if (i + 1) % args.progress == 0:
                self_fd = _count_fds('self')
                print(
                    f'[GUEST]    progress i={i + 1} self_fd_count={self_fd}',
                    flush=True,
                )
...
def _classify(ex: OSError, self_fd: int, guest_soft_rlimit: int) -> int:
    ...
    if ex.errno == errno.EMFILE:
        if self_fd < guest_soft_rlimit // 2:
            print(
                '[GUEST] EMFILE while self_fd_count is far below guest '
                'RLIMIT_NOFILE.'
            )
            print(
                '[GUEST] This is the signature predicted by the '
                'virtiofsd-RLIMIT hypothesis.'
            )
            print(
                '[GUEST] Check the [HOST] watch log: virtiofsd open_fds '
                'should be near its NOFILE soft.'
            )
            return 0
        print(
            '[GUEST] EMFILE but self_fd_count is also near guest RLIMIT; '
            'the guest process itself is the bottleneck.'
        )
        return 0
    if ex.errno == errno.ENFILE:
        print(
            '[GUEST] ENFILE = guest kernel file table exhausted. Check '
            '/proc/sys/fs/file-nr vs file-max.'
        )
        return 0
    print(
        f'[GUEST] unexpected errno {errno.errorcode.get(ex.errno, ex.errno)}; '
...
def main():
    ...
    d.add_argument(
        'path', help='virtiofs path (e.g. /mnt/aivm-persistent/<token>)'
    )
    d.add_argument(
        '--hold', type=int, default=5000, help='how many files to open + hold'
    )
    d.add_argument(
        '--max-walk',
        type=int,
